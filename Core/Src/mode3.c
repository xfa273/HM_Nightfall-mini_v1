    ...
    while (1) {
        ...
        case 0: { // 調整モード選択（0..7）
            ...
            case 8: // Straight test using case1 params (index0)
                // 直進テスト: mode3 の case1 で使用されるパラメータを参照
                apply_case_params_mode3_idx(0); // case1 -> index 0
                ...
                printf("[mode3-case8] Press RIGHT FRONT for VELOCITY (FR>1500), LEFT FRONT for DISTANCE (FL>1500) ...\n");
                while (1) {
                    if (ad_fr > 1500) {
            ...
            case 9: // Straight test using case7 params (index6)
                // 直進テスト: mode3 の case7 で使用されるパラメータを参照
                apply_case_params_mode3_idx(6); // case7 -> index 6
                ...
                // 最短走行ランタイム(run)と同一ロジックで任意パスを実行
                // 3区画直進: 6半区画 -> コード 200+6 = 206
                const uint16_t test_path_case9[] = { 206, 0 };
                ...
                // センサEnter待ち（右前:速度, 左前:距離）
                printf("[mode3-case9] Press RIGHT FRONT for VELOCITY (FR>1500), LEFT FRONT for DISTANCE (FL>1500) ...\n");
                while (1) {
                    if (ad_fr > 1500) {
                        log_print_velocity_all();
                        break;
